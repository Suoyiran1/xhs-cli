...
> 本项目核心浏览器自动化逻辑参考并移植自 [xpzouying/xiaohongshu-mcp](https://github.com/xpzouying/xiaohongshu-mcp)，将其从 MCP Server 架构改造为轻量 CLI 工具，去掉 Docker 和 mcporter 依赖，让 AI Agent 可以直接通过命令行调用。感谢原作者的开源贡献。
...
## 与 xiaohongshu-mcp 的关系

| | xiaohongshu-mcp | xhs-cli |
|---|---|---|
| 架构 | MCP Server + Docker | 单一 CLI 二进制 |
| 依赖 | Docker + mcporter | 仅 Chrome/Chromium |
| 调用方式 | MCP 协议 | 命令行 + JSON stdout |
| 适用场景 | MCP 生态集成 | AI Agent 命令行调用 |
| 核心逻辑 | 原创 | 移植自 xiaohongshu-mcp |
